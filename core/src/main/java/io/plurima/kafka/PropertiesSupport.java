 ...
 *
 * <p>Shared, non-{@code internal}-package copy logic for {@link PlurimaConsumerBuilder}
 * and {@link io.plurima.kafka.dlt.DltConfig} — both need this defensive copy, and neither
 * may depend on {@code io.plurima.kafka.internal} (ADR-012), so it lives here instead of
 * next to the (functionally identical) {@code internal.PropertiesCopy} used by the runtime
 * classes.
 *
 * <p>Public (not package-private) only because {@code DltConfig} lives in the
 * {@code io.plurima.kafka.dlt} subpackage and Java's package-private access does not
 * reach across packages; it is deliberately <strong>not</strong> annotated {@code @Stable}
 * — it is plumbing shared across this module's own packages, not a committed part of the
 * public API contract.
 */
@Experimental(
    reason = "Cross-package plumbing shared by PlurimaConsumerBuilder and DltConfig, not a"
        + " committed public API — signature/behavior may change without notice.")
